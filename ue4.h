...
static FVector GetCustomLocation(float Yaw, bool Forward)
{
    FRotator Rotation;
    ...
    if (Forward)
    {
        ...
        Rotation.Yaw = Yaw;
        auto Math = reinterpret_cast<UKismetMathLibrary*>(UKismetMathLibrary::StaticClass());
        return Math->STATIC_GetForwardVector(Rotation);
    }
    else
    {
        Rotation.Yaw = Yaw;
        auto Math = reinterpret_cast<UKismetMathLibrary*>(UKismetMathLibrary::StaticClass());
        return Math->STATIC_GetRightVector(Rotation);
    }
...
APlayerPawn_Athena_C* InitializePawn(AFortPlayerControllerAthena* PlayerController, FVector Loc = FVector(), FQuat Rot = FQuat(), bool open_glider = false)
{
    ...
    //ServerAcknowledgePossessionHook(PlayerController, Pawn);
    ...
    /*if (Pawn->bReplicateMovement = true && (bool)EMovementMode::MOVE_Custom) {
        Pawn->AddMovementInput(GetCustomLocation(Pawn->K2_GetActorRotation().Yaw, true), true, -1);
    }*/
